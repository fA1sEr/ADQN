...
class cheese95:
    def __init__(self):
        self.file = 'GameSimulator/cheese95.POMDP'
        self.states = 11
        self.actions = 4
        self.observations = 7
        self.cur_state = 0
        ...
        self.p_state = [0.100000,0.100000,0.100000,0.100000,0.100000,0.100000,0.100000,0.100000,0.100000,0.100000,0.0]
        self.total_steps = 0
        ...
        for line in lines:
            ...
            if T_flag!=0:
                ...
                for i in range(self.states):
                    self.T[T_last_action][T_flag-1][i] = float(line[i])
                T_flag += 1
                continue
            if O_flag!=0:
                line = line.split()
                for i in range(self.observations):
                    self.O[O_last_action][O_flag-1][i] = float(line[i])
                O_flag += 1
                continue
            if line[0]=='T':
                items = line.split(':')
                T_flag = 1
                T_last_action = int(items[1].strip())
            if line[0]=='O':
                items = line.split(':')
                O_flag = 1
                O_last_action = int(items[1].strip())
            # Rewards
            if line[0]=='R':
                items = line.split(':')
    ...
    def reset(self):
        ...
        p_obs = self.O[0,self.cur_state,:].flatten()
